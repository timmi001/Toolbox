...
- [Bundled PDF.js runtime](pdf-parser-runtime.md) — pdf-parse v2 needs native canvas support and an explicit worker URL when bundled by esbuild.
- [Rebase recovery](rebase-recovery.md) — if the rebase callback is unavailable, verify the branch ref and existing merge commit before aborting a duplicate detached rebase.
