...
export default function JpgToPdf() {
  ...
  return (
    <ToolLayout tool={tool} instructions="Upload one or more JPG images to combine them into a single PDF.">
      <input ref={inputRef} type="file" accept="image/jpeg,image/jpg" multiple className="hidden" onChange={(e) => e.target.files && setFiles(prev => [...prev, ...Array.from(e.target.files!)])} />
      <div onClick={() => inputRef.current?.click()} className="border-2 border-dashed border-border/50 rounded-xl p-10 text-center cursor-pointer hover:border-primary/50 transition-colors mb-4">
        ...
        <div className="font-medium">Click to add JPG images</div>
      </div>
      {files.length > 0 && (
