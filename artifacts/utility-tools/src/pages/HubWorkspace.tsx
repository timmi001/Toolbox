    ...
    <div className="min-h-[calc(100vh-76px)] bg-[#090D12] text-white">
      <div className="mx-auto flex min-h-[calc(100vh-76px)] max-w-[1480px]">
        <aside className="hidden w-64 shrink-0 border-r border-[#1B2936] bg-[#080C11] p-4 lg:block">
          <div className="mb-5 flex items-center justify-between"><div className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#718194]">Conversations</div><button type="button" className="rounded-lg p-2 text-[#8190A0] hover:bg-[#13202A] hover:text-white" aria-label="New conversation"><FileText className="h-4 w-4" /></button></div>
          <button type="button" className="mb-3 flex w-full items-center gap-2 rounded-xl border border-[#214238] bg-[#10271F] px-3 py-2.5 text-left text-xs font-semibold text-[#A9F2D8]"><Sparkles className="h-4 w-4" />New conversation</button>
          <div className="space-y-1">{['Getting started', 'Content ideas', 'Research notes'].map((item, index) => <button key={item} type="button" className={`flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-left text-xs ${index === 0 ? 'bg-[#14222D] text-white' : 'text-[#7F8FA1] hover:bg-[#111B25] hover:text-white'}`}><span className="h-1.5 w-1.5 rounded-full bg-[#4FD9B0]" />{item}</button>)}</div>
          ...
          <div className="flex flex-1 flex-col px-4 pb-4 pt-5 sm:px-8">
            <div className="mx-auto flex w-full max-w-3xl flex-1 flex-col">
              ...
              <div className="sticky bottom-3 mt-auto pt-5">
                ...
                {error && <div className="mb-3 flex items-center justify-between gap-3 rounded-xl border border-red-900/60 bg-red-950/30 px-3 py-2 text-xs text-red-200"><span>{error}</span><button type="button" onClick={sendMessage} className="font-semibold text-white underline">Retry</button></div>}
                {loading && <div className="mb-3 text-xs text-[#A9F2D8]">Toolbuxx AI is thinking…</div>}
                <div className="rounded-2xl border border-[#2A3A48] bg-[#0E151D] p-2 shadow-[0_12px_40px_rgba(0,0,0,0.3)] focus-within:border-[#3DDBC0]/70"><textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); sendMessage(); } }} rows={3} placeholder="Ask anything…" className="w-full resize-none bg-transparent px-3 py-2 text-sm leading-6 text-white outline-none placeholder:text-[#68798A]" /><div className="flex items-center justify-between gap-2 px-2 pb-1"><div className="flex items-center gap-1"><button type="button" className="rounded-lg p-2 text-[#8190A0] hover:bg-[#17242F] hover:text-white" aria-label="Attach file"><Paperclip className="h-4 w-4" /></button><button type="button" className="rounded-lg p-2 text-[#8190A0] hover:bg-[#17242F] hover:text-white" aria-label="Voice input"><Mic className="h-4 w-4" /></button><button type="button" className="rounded-lg px-2 py-1.5 text-[11px] text-[#9EACBB] hover:bg-[#17242F] hover:text-white">Toolbuxx AI ▾</button></div><div className="flex items-center gap-1"><button type="button" className="rounded-lg px-2 py-1.5 text-[11px] text-[#9EACBB] hover:bg-[#17242F] hover:text-white">Balanced ▾</button><button type="button" onClick={sendMessage} className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#10B981] text-[#061410] transition hover:bg-[#34D399]" aria-label="Send message"><ArrowUp className="h-4 w-4" /></button></div></div></div>
