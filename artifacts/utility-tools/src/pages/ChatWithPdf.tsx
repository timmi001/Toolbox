  ...
  return (
    <>
      ...
      >
        ...
        >
          <header className="relative z-30 flex h-14 shrink-0 items-center justify-between gap-2 border-b border-[#1a1a1a] bg-[#000000]/95 px-3 backdrop-blur-xl sm:px-5">
          ...
          {displayView === 'document' && activeDocument ? (
            ...
            <>
              ...
              <div className="pointer-events-none sticky bottom-0 z-20 mt-auto px-3 pb-[max(env(safe-area-inset-bottom),0.65rem)] pt-10 sm:px-5">
                  <div className="pointer-events-auto relative mx-auto w-full max-w-3xl">
                    ...
                    <div className="flex min-w-0 items-center gap-2 rounded-2xl border border-[#1a1a1a] bg-[#000000]/95 p-2 shadow-[0_-8px_26px_rgba(0,0,0,0.22)] backdrop-blur-xl">
                      <button type="button" data-testid="button-open-pdf-add-menu" aria-label="Add PDF or attachment" onClick={() => setOpenMenu((current) => current === 'add' ? null : 'add')} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-[#303030] text-[#b9dce9] transition hover:bg-white/[0.07]"><Plus className="h-4 w-4" /></button>
                      ...
                      <button type="button" data-testid="button-open-pdf-tools" aria-label="Open PDF tools" onClick={() => setOpenMenu((current) => current === 'tools' ? null : 'tools')} disabled={!activeDocument} className="flex h-9 shrink-0 items-center gap-1 rounded-xl border border-[#303030] px-2.5 text-[11px] font-medium text-[#9fc8d6] transition hover:bg-white/[0.07] disabled:cursor-not-allowed disabled:opacity-40"><MoreHorizontal className="h-4 w-4" />Tools</button>
